...
describe('PrivyAuthButton', () => {
  ...
  it('should call login when clicked', async () => {
    ...
    expect(mockPrivyState.login).toHaveBeenCalled();
  });

  it('should still show connect wallet button when authenticated', () => {
    mockPrivyState = createMockAuthenticatedPrivy();
    
    render(<PrivyAuthButton />);
    
    // The button should still show "Connect Wallet" even when authenticated
    // as this component is specifically for wallet connection
    expect(screen.getByText('Connect Wallet')).toBeInTheDocument();
  });
});
...
describe('AuthGuard', () => {
  ...
  it('should render children for authenticated users regardless of requireKYC setting', async () => {
    mockPrivyState = createMockAuthenticatedPrivy();
